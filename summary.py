...
results_collection = db['results']  # Replace 'results' with your results collection name
...
def main():
    ...
    # Session state to track which document's summary is currently being viewed
    if 'selected_document_id' not in st.session_state:
        st.session_state.selected_document_id = None  # No document selected by default
            ...
            for result in results:
                ...
                try:
                    ...
                    if st.button(f"View Summary and Keywords for {result['document_name']}"):
                        st.session_state.selected_document_id = str(result['_id'])  # Store as string for later use
                        st.rerun()  # Re-run the app to load the new summary view
    ...
    # Check if a document is selected to view its summary
    if st.session_state.selected_document_id:
        selected_document = results_collection.find_one({"_id": ObjectId(st.session_state.selected_document_id)})

        if selected_document:
            st.title("Document Summary")
            st.subheader("Summary:")
            st.write(selected_document['summary'])
            st.subheader("Keywords:")
            st.write(selected_document['keywords'])
            st.write(f"**Document Name:** {selected_document['document_name']}")
            st.write(f"**Path:** {selected_document['path']}")
            st.write(f"**Size (MB):** {selected_document['size_mb']}")

            # Button to close the summary view
            if st.button("Close Summary"):
                st.session_state.selected_document_id = None  # Clear the selected document ID
                st.rerun()  # Re-run the app to go back to the metadata list
